...
export const Notification = ({ onClearNotification }) => {
  ...
  useEffect(() => {
    ...
    const fetchNotificationData = async () => {
      try {
        const api = "http://172.235.21.99:3105/hiring/auth/notificationforadmin"
    ...
    const fetchRecruiterNotification = async () => {
      try {
        const token = localStorage.getItem('accessToken');
        const api = "http://172.235.21.99:3105/hiring/interviewer/notshortlistedNotification"
    ...
    const fetchInterviewerNotification = async () => {
      try {
        const api = "http://172.235.21.99:3105/hiring/entryLevel/getMeetingNotification"
        const token = localStorage.getItem('accessToken');
        const response = await axios.get(api,
          {
            headers: {
    ...
    if (localStorage.getItem('role') == 'ROLE_ADMIN') {
      try {
        ...
        const response = await axios.post(
          'http://172.235.21.99:3105/hiring/auth/clearnotification',
          null,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      try {
        const token = localStorage.getItem('accessToken');
        const response = await axios.post(
          'http://172.235.21.99:3105/hiring/interviewer/clearNotification',
          null,
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      try {
        const token = localStorage.getItem('accessToken');
        const response = await axios.post(
          'http://172.235.21.99:3105/hiring/entryLevel/clearNotification',
          null,
          {
            headers: {
              Authorization: `Bearer ${token}`,
